...
public class Player implements ImageObserver {
	...
	public void moveShield(int speed) {
		...
		int buffer = 5; // holds variable for distance from center of ship to
						// shield
		...
		// find new angle
		double counterTheta = curTheta + deltaTheta; // for counter clockwise
														// movement
		// double clockwiseTheta = curTheta - deltaTheta; //for clockwise
		// movement
		...
		/*
		 * another formula which might work deltaX = oX-cX; deltaY = oY-cY;
		 * radius = sqrt(deltaX*deltaX+deltaY*deltaY); orthoX =
		 * -deltaY*d/radius; orthoY = deltaX*d/radius; newDeltaX =
		 * deltaX+orthoX; newDeltaY = deltaY+orthoY; newLength =
		 * sqrt(newDeltaX*newDeltaX+newDeltaY*newDeltaY); aX =
		 * cX+newDeltaX*radius/newLength; aY = cY+newDeltaY*radius/newLength;
		 */
	}

	public void paint(Graphics g) throws IOException {
		shp = ImageIO.read(new File("Images/orangeship.png"));
		g.drawImage(shp, ship.getX(), ship.getY(), ship.getWidth(), ship.getHeight(), this);
